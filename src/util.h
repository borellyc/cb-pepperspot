// $Id: util.h 1.54 2006-02-22 16:21:55+02:00 anttit@tcs.hut.fi $
...
#define IPV6_RECVHOPLIMIT \
  IPV6_HOPLIMIT //!< IPv6 socket option to get/set hop limit value
#endif

#ifndef IPV6_RECVPKTINFO
#define IPV6_RECVPKTINFO \
  IPV6_PKTINFO //!< IPv6 socket option to receive packet information
#endif
...
//!
static inline void ipv6_addr_set(struct in6_addr *addr, uint32_t w1,
...
static inline void ipv6_addr_llocal(const struct in6_addr *addr,
  ...
  ipv6_addr_set(llocal, htonl(0xFE800000), 0, ((uint32_t *)addr->s6_addr)[2],
                ((uint32_t *)addr->s6_addr)[3]);
}
...
static inline void free_iov_data(struct iovec *iov, int count) {
  ...
  if (iov == NULL) return;
  while (len--) {
    if (iov[len].iov_base) free(iov[len].iov_base);
  }
}
