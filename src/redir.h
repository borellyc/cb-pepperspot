...
#define REDIR_HTTP_SELECT_TIME \
  500000 //!< HTTP select() timeout in microseconds = 0.5 seconds

#define REDIR_RADIUS_MAX_TIME 60 //!< Timeout for RADIUS request in seconds
#define REDIR_RADIUS_SELECT_TIME \
  500000 //!< RADIUS select() timeout in microseconds = 0.5 seconds
...
struct redir_conn_t {
  ...
  char redirurlbuf[RADIUS_ATTR_VLEN +
                   1]; //!< Redirection URL obtained from radius server
  ...
  uint8_t classbuf[RADIUS_ATTR_VLEN + 1]; //!< Class attribute received from
                                          //! radius server and used in
...
struct redir_t {
  ...
  struct sockaddr_storage
      radiuslisten; //!< Listen address to communicate with RADIUS server
  struct sockaddr_storage radiusserver0; //!< Address of primary RADIUS server
  struct sockaddr_storage
      radiusserver1;       //!< Address of secondary RADIUS server
...
//!  \param redir pointer will be filled with newly redirection manager if
//!  success
...
//!
//!  \brief Set callback to determine state information for the IPv4
//!  connection.
//!  \param redir redir_t instance
...
// [SV]
//!
//!  \brief Set callback to determine state information for the IPv6
//!  connection.
//!  \param redir redir_t instance
