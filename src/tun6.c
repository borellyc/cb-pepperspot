...
#define TUN_HEAD_IPV6_INITIALIZER \
  { 0, htons(ETH_P_IPV6) }
#define tun_head_is_ipv6(h) (h.proto == htons(ETH_P_IPV6))
    ...
    defined(__DragonFly__) || defined(__APPLE__) // Darwin
//!
...
//!
//!  Originally there was strlcpy but it lacks in Linux libc... replace this
//!  latter!
//!  for the moment cross the finger... this function is not safe...
//!
#define safe_strcpy(tgt, src) (strncpy(tgt, src, sizeof(tgt)))
...
static int proc_write_zero(const char *path) {
  ...
  if (write(fd, "0", 1) != 1) retval = -1;
  if (close(fd)) retval = -1;
...
static void plen_to_mask(unsigned plen, struct in6_addr *mask) {
  ...
  while (i < d.quot) mask->s6_addr[i++] = 0xff;

  if (d.rem) mask->s6_addr[i++] = 0xff << (8 - d.rem);

  while (i < 16) mask->s6_addr[i++] = 0;
}

//!
//!  Converts a prefix length to a struct sockaddr_in6 (used for the BSD
//!  routing)
//!
...
static int tun6_set_interface_flags(struct tun6_t *this, int flags) {
  ...
  if (if_indextoname(this->ifindex, ifr.ifr_name) == NULL) { return -1; }
...
static struct tun6 *tun6_create(const char *req_name) {
  ...
  if (fd == -1) {
    syslog(LOG_ERR, "Tunneling driver error (%s): %s", tundev,
           strerror(errno));
    (void)close(reqfd);
    free(t);
    return NULL;
  ...
  if (ioctl(fd, TUNSETIFF, (void *)&req)) {
    syslog(LOG_ERR, "Tunneling driver error (%s): %s", "TUNSETIFF",
           strerror(errno));
    goto error;
  }
  ...
  if (fd == -1) {
    syslog(LOG_ERR, "Tunneling driver error (%s): %s", "/dev/tun*",
           strerror(errno));
    goto error;
  } else {
    struct stat st;
    ...
    if (safe_strcpy(t->orig_name, name)) goto error;
#endif
  }

  int id = if_nametoindex(t->orig_name);
  if (id == 0) {
    syslog(LOG_ERR, "Tunneling driver error (%s): %s", t->orig_name,
           strerror(errno));
    goto error;
  }
  ...
  if (ioctl(fd, TUNSIFHEAD, &(int){1})) {
    syslog(LOG_ERR, "Tunneling driver error (%s): %s", "TUNSIFHEAD",
           strerror(errno));
  ...
  if (ioctl(fd, TUNSLMODE, &(int){0})) {
    syslog(LOG_ERR, "Tunneling driver error (%s): %s", "TUNSLMODE",
           strerror(errno));
    goto error;
  }
#endif
  ...
  if (req_name != NULL) {
    ...
    if (if_indextoname(id, req.ifr_name) == NULL) {
      syslog(LOG_ERR, "Tunneling driver error (%s): %s", "if_indextoname",
             strerror(errno));
      goto error;
...
#else
      syslog(LOG_DEBUG,
             "Tunnel interface renaming is not supported on your operating "
             "system.\n"
             "To run miredo or isatapd properly, you need to remove the\n"
             "InterfaceName directive from their respective configuration "
             "file.\n");
      errno = ENOSYS;
#endif
      {
        syslog(LOG_ERR, "Tunneling driver error (%s): %s", "SIOCSIFNAME",
               strerror(errno));
        goto error;
      }
    }
...
static int tun6_iface_addr(int reqfd, int id, int add,
  ...
  if ((prefix_len > 128) || (addr == NULL)) return -1;
  ...
  if (add) {
    ...
    if (if_indextoname(id, r.addreq6.ifra_name) == NULL) return -1;
    ...
    if (if_indextoname(id, r.delreq6.ifr_name) == NULL) return -1;
...
//!
static int tun6_add_address(struct tun6 *t, const struct in6_addr *addr,
                            unsigned prefixlen) {
  assert(t != NULL);
...
#if defined(USE_LINUX)
  char ifname[IFNAMSIZ];
  if ((res == 0) && (if_indextoname(t->id, ifname) != NULL)) {
    ...
    snprintf(proc_path + 24, sizeof(proc_path) - 24, "%s/accept_redirects",
             ifname);
    proc_write_zero(proc_path);
  }
#endif
...
static inline int tun6_recv_inner(int fd, void *buffer, size_t maxlen) {
  ...
  if ((len < (int)sizeof(head)) || !tun_head_is_ipv6(head))
...
int tun6_decaps(struct tun6_t *this) {
  ...
  if ((status = tun6_recv_inner(this->device->fd, buffer, sizeof(buffer))) ==
      -1) {
    return 0;
  }

  if (this->cb_ind6) { return this->cb_ind6(this, buffer, status); }
  return 0;
