...
// Manually modified for details by cb 2013-09-25
// TODO : Regenerate this file with gengetopt
...
struct gengetopt_args_info {
  ...
  int debugfacility_arg;    //!< Which modules to print debug messages for
                            //!(default='1')
  char *debugfacility_orig; //!< Which modules to print debug messages for
                            //! original value given at command line
  char *conf_arg;  //!< Read configuration file (default='/etc/pepper.conf')
  char *conf_orig; //!< Read configuration file original value given at command
                   //! line
  int interval_arg;    //!< Re-read configuration file at this interval
                       //!(default='3600')
  char *interval_orig; //!< Re-read configuration file at this interval
                       //! original value given at command line
  char *pidfile_arg;   //!< Filename of process id file
                       //!(default='/var/run/pepper.pid')
  char *pidfile_orig;  //!< Filename of process id file original value given at
                       //! command line
  char *ipversion_arg; //!< Allowed version of IP (default='6')
  char *ipversion_orig; //!< Allowed version of IP (default='6') value given at
                        //! command line
  char *statedir_arg;   //!< Directory of nonvolatile data
                        //!(default='/var/lib/pepper/')
  char *statedir_orig;  //!< Directory of nonvolatile data original value given
                        //! at command line
  char *net_arg;        //!< Network (default='192.168.182.0/24')
  char *net_orig;       //!< Network original value given at the command line
  char *dynip_arg;      //!< Dynamic IP address pool
  char *dynip_orig;     //!< Dynamic IP address pool original value given at
                        //! command line
  char *statip_arg;     //!< Static IP address pool
  char *statip_orig;    //!< Static IP address pool original value given at
                        //! command line
  char *dns1_arg;       //!< Primary DNS server IP address
  char *dns1_orig; //!< Primary DNS server IP address original value given at
                   //! command line
  char *dns2_arg;  //!< Secondary DNS server IP address
  char *dns2_orig; //!< Secondary DNS server IP address original value given at
                   //! command line
  ...
  char *domain_arg;      //!< Domain to use for DNS lookups
                         //!(default='key.pepperspot.info')
  char *domain_orig; //!< Domain to use for DNS lookups original value given at
                     //! command line
  char *ipup_arg;    //!< Script to run after link-up
  char *ipup_orig;   //!< Script to run after link-up original value given at
                     //! command line
  char *ipdown_arg;  //!< Script to run after link-down
  char *ipdown_orig; //!< Script to run after link-down original value given at
                     //! command line
  char *conup_arg;   //!< Script to run after user logon
  char *conup_orig; //!< Script to run after user logon original value given at
                    //! command line
  char *condown_arg;  //!< Script to run after user logoff
  char *condown_orig; //!< Script to run after user logoff original value given
                      //! at command line
  char *radiuslisten_arg;  //!< IP address to send from
  char *radiuslisten_orig; //!< IP address to send from original value given at
                           //! command line
  char *radiusserver1_arg; //!< IP address of radius server 1
                           //!(default='rad01.hotradius.com')
  char *radiusserver1_orig;  //!< IP address of radius server 1 original value
                             //! given at command line
  char *radiusserver2_arg;   //!< IP address of radius server 2
                             //!(default='rad02.hotradius.com')
  char *radiusserver2_orig;  //!< IP address of radius server 2 original value
                             //! given at command line
  int radiusauthport_arg;    //!< Authentication UDP port of radius server
                             //!(default='1812')
  char *radiusauthport_orig; //!< Authentication UDP port of radius server
                             //! original value given at command line
  int radiusacctport_arg;    //!< Accounting UDP port of radius server
                             //!(default='1813')
  char *radiusacctport_orig; //!< Accounting UDP port of radius server original
                             //! value given at command line
  char *radiussecret_arg;    //!< Radius shared secret
  char *radiussecret_orig;   //!< Radius shared secret original value given at
                             //! command line
  char *radiusnasid_arg;     //!< Radius NAS-Identifier (default='nas01')
  char *radiusnasid_orig;    //!< Radius NAS-Identifier original value given at
                             //! command line
  char *radiusnasip_arg;     //!< Radius NAS-IP-Address
  char *radiusnasip_orig;    //!< Radius NAS-IP-Address original value given at
                             //! command line
  char *radiuscalled_arg;    //!< Radius Called-Station-ID
  char *radiuscalled_orig;   //!< Radius Called-Station-ID original value given
                             //! at command line
  char *radiuslocationid_arg;    //!< WISPr Location ID
  char *radiuslocationid_orig;   //!< WISPr Location ID original value given at
                                 //! command line
  char *radiuslocationname_arg;  //!< WISPr Location Name
  char *radiuslocationname_orig; //!< WISPr Location Name original value given
                                 //! at command line
  int radiusnasporttype_arg;     //!< Radius NAS-Port-Type (default='19')
  char *radiusnasporttype_orig;  //!< Radius NAS-Port-Type original value given
                                 //! at command line
  int coaport_arg;    //!< Radius disconnect port to listen to (default='0')
  char *coaport_orig; //!< Radius disconnect port to listen to original value
                      //! given at command line
  int coanoipcheck_flag; //!< Allow radius disconnect from any IP (default=off)
  char *proxylisten_arg; //!< IP address to listen to
  char *proxylisten_orig;  //!< IP address to listen to original value given at
                           //! command line
  int proxyport_arg;       //!< UDP port to listen to (default='0')
  char *proxyport_orig;    //!< UDP port to listen to original value given at
                           //! command line
  char *proxyclient_arg;   //!< IP address of proxy client(s)
  char *proxyclient_orig;  //!< IP address of proxy client(s) original value
                           //! given at command line
  char *proxysecret_arg;   //!< Radius proxy shared secret
  char *proxysecret_orig;  //!< Radius proxy shared secret original value given
                           //! at command line
  char *confusername_arg;  //!< Username for remote config
  char *confusername_orig; //!< Username for remote config original value given
                           //! at command line
  char *confpassword_arg;  //!< Password for remote config
  char *confpassword_orig; //!< Password for remote config original value given
                           //! at command line
  char *dhcpif_arg;        //!< Local Ethernet interface
  char *dhcpif_orig;     //!< Local Ethernet interface original value given at
                         //! command line
  char *dhcpmac_arg;     //!< Interface MAC address
  char *dhcpmac_orig;    //!< Interface MAC address original value given at
                         //! command line
  char *dhcplisten_arg;  //!< IP address for listenning
  char *dhcplisten_orig; //!< IP address original value given at command line
  int lease_arg;         //!< Lease time to allocate to clients (default='600')
  char *lease_orig; //!< Lease time to allocate to clients original value given
                    //! at command line
  ...
  char *uamserver_orig;  //!< URL of authentication web server original value
                         //! given at command line
  char *uamserver6_arg;  //!< IPv6 UAM server authentication
  char *uamserver6_orig; //!< IPv6 UAM server authentication original value
                         //! given at command line
  char *uamhomepage_arg; //!< URL of homepage to redirect unauthenticated users
                         //! to
  char *uamhomepage_orig; //!< URL of homepage to redirect unauthenticated
                          //! users to original value given at command line
  char *uamsecret_arg;    //!< Shared secret between uamserver and pepper
  char *uamsecret_orig;   //!< Shared secret between uamserver and pepper
                          //! original value given at command line
  char *uamlisten_arg; //!< IP address to listen to for authentication requests
  char *uamlisten_orig;   //!< IP address to listen to for authentication
                          //! requests original value given at command line
  int uamport_arg;        //!< TCP port to bind to for authentication requests
                          //!(default='3990')
  char *uamport_orig;     //!< TCP port to bind to for authentication requests
                          //! original value given at command line
  char **uamallowed_arg;  //!< Domain names exempt from access check
  char **uamallowed_orig; //!< Domain names exempt from access check  original
                          //! value given at command line
  ...
  char **macallowed_orig; //!< List of allowed MAC addresses original value
                          //! given at command line
  char *macsuffix_arg;    //!< Suffix to add to the MAC address
  char *macsuffix_orig;   //!< Suffix to add to the MAC address original value
                          //! given at command line
  char *macpasswd_arg;    //!< Password used when performing MAC authentication
                          //!(default='password')
  char *macpasswd_orig;   //!< Password used when performing MAC authentication
                          //! original value given at command line
  ...
  int ipversion_given;           //!< Wheter ipversino was given
  ...
  int dhcplisten_given;          //!< Whether dhcplisten was given
  int staticipv6_given;          //!< Whether staticipv6 was given
  int ipv6prefix_given;          //!< Whether ipv6prefix was given
  ...
  int uamserver6_given;          //!< Whether uamserver6 was given
